...
> **The team's demo machine is MacBook Pro M3 Pro · 18 GB unified memory** (verified 2026-05-09 via `sysctl hw.memsize` → Mac15,6 / MRX33T/A). That puts us at the brief's **Minimum tier (16 GB)**. We don't have an EdgeXpert. The pitch frames our 18 GB M3 Pro as the proof point — *literally the team the brief describes* — with MSI AI Edge PC / Copilot+ Prestige/PRO framed as the deployment target where the same architecture flies. **OUT-of-budget on this hardware: gpt-oss:20b, medgemma:27b, devstral:24b, mistral-small:22b.** In-budget: gemma3:4b, phi4-mini, qwen3:4b, gemma3n:e4b — all at Q4_K_M. See [02-specification.md §"Hardware Reality (verbatim)"](02-specification.md) and the `feedback_hardware_first` memory.
